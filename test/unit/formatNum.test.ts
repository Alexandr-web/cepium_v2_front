...
describe("formatNum", () => {
	...
	describe("обработка строковых значений", () => {
		...
		it("строка с запятой как разделителем корректно парсится (запятая заменяется на точку)", () => {
			expect(replaceSpaces(formatNum("123,45"))).toBe("123,45");
		});
	});
	...
	describe("крайние случаи и граничные значения", () => {
		...
		it("очень маленькое положительное число (расширенная точность для |num| < 0.01)", () => {
			expect(replaceSpaces(formatNum(0.0001))).toBe("0,0001");
			expect(replaceSpaces(formatNum(0.004))).toBe("0,004");
			expect(replaceSpaces(formatNum(0.005))).toBe("0,005");
		});
